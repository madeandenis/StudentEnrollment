...
export function StudentFormModal({ opened, studentId, onClose }: StudentFormModalProps) {
    ...
    const { data: student, isLoading: isLoadingStudent, isError: isStudentError } = useStudentDetails(studentId || 0);
    ...
    const form = useForm<CreateStudentRequest | UpdateStudentRequest>({
        ...
        validate: {
            ...
            address: {
                address1: (value: string) => validators.required(value, 'Adresa este obligatorie'),
                city: (value: string) => validators.required(value, 'Orașul este obligatoriu'),
                country: (value: string) => validators.required(value, 'Țara este obligatorie'),
    ...
    const handleSubmit = async (values: CreateStudentRequest | UpdateStudentRequest) => {
        clearErrors();
    ...
    const isPending = isEditMode ? updateStudentMutation.isPending : createStudentMutation.isPending;
