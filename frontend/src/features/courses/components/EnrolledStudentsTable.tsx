import { Table, Text, ActionIcon, Group, Tooltip, Badge, Center, Anchor } from '@mantine/core';
...
export function EnrolledStudentsTable({
    ...
    const formatGrade = (grade?: number) => {
        ...
        return (
            <Badge variant="light" color={grade >= 5 ? 'green' : 'red'} size="md" radius="sm">
                {grade.toFixed(2)}
            </Badge>
        );
    };
                ...
                <Table.Thead bg="gray.0">
                    <Table.Tr>
                        <Table.Th style={{ width: 140 }}>
                            <Text size="xs" fw={600} tt="uppercase" c="dimmed">Cod Student</Text>
                        </Table.Th>
                        <Table.Th>
                    ...
                    {students.length === 0 ? (
                        ...
                        students.map((student) => (
                            <Table.Tr key={student.studentId}>
                                ...
                                <Table.Td>
                                    <Text size="xs" c="dimmed">{formatDate(student.enrollmentDate)}</Text>
                                </Table.Td>
                                <Table.Td>
                                    <Group gap="xs">
                                        {formatGrade(student.grade)}
                                        {student.assignedByProfessor && (
                                            <Tooltip label={`Atribuită de: ${student.assignedByProfessor}`}>
                                ...
                                {canAssignGrade && (
                                    <Table.Td>
                                        <Group gap={4} justify="center" wrap="nowrap">
                                            <Tooltip label={student.grade ? "Modifică nota" : "Adaugă notă"}>
                                                <ActionIcon
                                                    variant="subtle"
                                                    color="blue"
                                                    size="sm"
