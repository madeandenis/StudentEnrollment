...
export function CourseFormModal({ opened, courseId, onClose }: CourseFormModalProps) {
    ...
    const { data: course, isLoading: isLoadingCourse, isError: isCourseError } = useCourseDetails(courseId || 0);
    ...
    const handleSubmit = async (values: CreateCourseRequest | UpdateCourseRequest) => {
        clearErrors();
    ...
    const isPending = isEditMode ? updateCourseMutation.isPending : createCourseMutation.isPending;
