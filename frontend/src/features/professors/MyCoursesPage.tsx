...
export function MyCoursesPage() {
  ...
  const {
    ...
    error: errorAssigned,
  } = useProfessorAssignedCourses(user?.professorCode);
  ...
  const openUnassignModal = (courseId: number) => {
    const course = assignedData?.assignedCourses.find(
    ...
    try {
      ...
      notifications.show({
        ...
        message:
          ...
          "A apărut o eroare la alocarea cursului.",
        color: "red",
      });
    }
  };
  ...
  const handleConfirmUnassign = async () => {
    ...
    try {
      ...
      notifications.show({
        ...
        message:
          error?.response?.data?.detail ||
          "A apărut o eroare la renunțarea cursului.",
        color: "red",
      });
    }
  };
  ...
  return (
    <Stack gap="lg">
      ...
      <Paper p="lg" shadow="xs" withBorder bg="white" radius="md">
        <Stack gap="lg">
          <Group justify="space-between" align="center">
            <div>
              <Title order={3} c="dark.4">
                Cursuri fără titular
              </Title>
              <Text size="sm" c="dimmed">
                Următoarele cursuri nu au încă un profesor alocat. Te poți
                înscrie ca titular.
              </Text>
            </div>
          </Group>
      ...
      <ConfirmModal
        ...
        description={(name) =>
          `Ești sigur că vrei să renunți la cursul "${name}"? Această acțiune nu poate fi anulată.`
        }
