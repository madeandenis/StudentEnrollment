...
export function ProfessorAssignedCoursesSection({
  ...
  const { data, isLoading, isError, error } =
    useProfessorAssignedCourses(professorId);
  ...
  if (isError) {
    return (
      <Paper p="md" shadow="sm" withBorder>
        <ErrorAlert
          errors={
            error?.message || "Nu s-a putut încărca cursurile profesorului"
          }
          onReload
        />
      </Paper>
            ...
            <Group>
              ...
              <div>
                ...
                <Text size="sm" c="dimmed">
                  Total cursuri: <strong>{totalCourses}</strong> • Total
                  studenți: <strong>{totalStudents}</strong>
                </Text>
              </div>
            </Group>
            {isAdmin && (
