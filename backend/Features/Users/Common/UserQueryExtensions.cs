...
public static class UserQueryExtensions
{
    extension(IQueryable<ApplicationUser> query)
    {
        ...
        public IQueryable<ApplicationUser> ApplyRoleFilter(GetUserListRequest request)
        {
            if (request.IsAdmin.HasValue)
                query = request.IsAdmin.Value
                    ? query.Where(u =>
                        u.UserRoles.Any(ur => Roles.AdminRoles.Contains(ur.Role.Name))
                    )
                    : query.Where(u =>
                        !u.UserRoles.Any(ur => Roles.AdminRoles.Contains(ur.Role.Name))
                    );
        ...
        public IQueryable<ApplicationUser> ApplyProfessorFilter(
            GetUserListRequest request,
            IQueryable<Professor> professors
        )
        {
            if (request.ExcludeWithProfessor == true)
                query = query.Where(u => !professors.Select(p => p.UserId).Contains(u.Id));
